...
export default class ChatMessageEditor extends React.PureComponent {
    ...
    */
	__send_message = () => {
        ...
        var response = this.__fetchResponse(this.input.current.value);
        response = response.trimLeft().trimRight();
        if (response) {
            this.props.sendMessage (DateTime.get_datetime (), true, response);
        }
    ...
    /*
    * @description: Fetches the response from the server.
    * @parameters: input
    * return data
    */
    __fetchResponse = async (input) => {
